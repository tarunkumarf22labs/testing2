...
import { SearchLocationProps, SearchedLocationsProps, SearchedVillaCardProps } from 'src/Props/Search';
...
function MobileMap({location , data , villasResult , handleClick , cardTopPosition  } : IMobileMap) {
  ...
  return (
    ...
    {showSearch && (
      <>
        <div className="fixed w-full h-full inset-0 z-[1000000] " onClick={handleshowSearch}  ></div>
        <div className=" z-[1000000] absolute w-full top-[50px]" >
          <Search locations={SearchLocationProps(
            data.states,
            data.countries,
            data.cities
          )}
    ...
    />
    {isMobile && (<BottomSheet isOpen={isOpen} handleclose={() => setOpen(false)}  >
