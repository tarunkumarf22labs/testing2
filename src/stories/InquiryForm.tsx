...
const InquiryForm = ({ props }: IInquiryForm) => {
  ...
  const { numberOfGuests, startDate, endDate } = useContext(AppContext);
  ...
  const [guestsNumber, setGuestsNumber] = useState({
    ...
    children: numberOfGuests.children > 0 ? numberOfGuests.children : null,
    infants: numberOfGuests.infants > 0 ? numberOfGuests.infants : null,
    pets: numberOfGuests.pets > 0 ? numberOfGuests.pets : null,
    staff:
      numberOfGuests.additional_guests > 0
  ...
  const submitData = async (body) => {
    try {
      const { data, error, status } = await NetWrapper('api/bookingrequests', {
        method: 'POST',
  ...
  const handleFormSubmit = (e) => {
    ...
    let body = {
      data: {
        ...
        villa: location,
        checkIn: dates.firstDate,
        checkOut: dates.lastDate,
        // noOfAdults: guestsNumber.adults,
        // noOfChildren: guestsNumber.children,
        noOfPets: guestsNumber.pets,
        noOfAccompanyingStaff: guestsNumber.staff,
        ...
        <div>
          ...
          <div className="w-full m-auto mb-8">
            <select
              ...
              id=""
              value={location}
              className="w-full text-xs sm:text-sm  uppercase border-none outline-none focus:ring-0 custom-select text-[#545456]"
              onChange={(e) => {
