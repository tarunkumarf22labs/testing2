...
function Paymentcard({ datas }) {
  const calculationdata = datas.map((data) => {
    return (
      <div className="grid grid-cols-2" key={data.title}>
        <h2 className="text-[#545456] font-Brandon font-normal text-sm md:tracking-wider whitespace-nowrap">
          {data.title}
        </h2>
        <h2 className="text-[#545456] font-Brandon font-normal text-sm place-self-end">
          {data.price}
        </h2>
      </div>
    );
  ...
  return (
    <div className=" w-full   xl:w-[350px]  ">
      ...
      <h2 className="uppercase -tracking-[0.64px] text-[32px] pb-[27.5px] font-light">
        summary
      </h2>
